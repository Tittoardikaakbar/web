      ...
      if ($user_id == '') {
         ...
         if ($select_orders->rowCount() > 0) {
            while ($fetch_orders = $select_orders->fetch(PDO::FETCH_ASSOC)) {
               $payment_status = isset($fetch_orders['payment_status']) ? $fetch_orders['payment_status'] : 'N/A'; // Tambahkan pengecekan ini
   ?>
   <div class="box">
      ...
      <p>Status Pembayaran : <span style="color:<?php if($payment_status == 'pending'){ echo 'red'; } else { echo 'green'; }; ?>"><?= $payment_status; ?></span> </p>
   </div>
   <?php
            }
         } else {
